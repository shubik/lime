...
Dispatcher.prototype.on = function() {
    ...
    /*
    We might want to allow passing options, e.g.
      - replace current listener (as in old Dispatcher)
      - use beginning and end of input in matching event names (because by default it will match parts inside string)
    If we do, than adding such listeners with same event name will not work, so such events should be stored separately
    */
