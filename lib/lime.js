...
Dispatcher = function(options) {
    ...
    this._sub.on('message', function(ch, data) {
        ...
        _.each(self._callbacks, function(callbacks, key) {
            // should map wach section of key in separate regex tests
            // exec callbacks only if all key parts are matched

            var pattern = new RegExp(key.replace(/\*/gi, '(.*)', 'gi'), 'gi');
            payload.key.match(pattern) ? execCallbacks(callbacks, payload.key.split(self.options.delimeter), payload.data) : 0;
        });
    });
}
